...
import Comment from "../models/chill/CommentModels.js";
import Reel from "../models/chill/ReelModels.js";
import User from "../models/chill/UserModels.js";
  ...
  try {
    ...
    if (user.mode === "study") {
      return res.status(403).json({
        success: false,
        message:
          "Cannot create reels in study mode. Switch to chill mode first!",
...
export const getReels = async (req, res) => {
  try {
    ...
    const reels = await Reel.find(filter)
      .populate("user", "name username avatar mode")
      .populate({
        path: "comments",
        populate: {
          path: "user",
    ...
    const reelsWithStatus = reels.map((reel) => {
      const isLiked = req.user ? reel.likes.includes(req.user.id) : false;
      const isSaved = req.user ? reel.saves.includes(req.user.id) : false;
