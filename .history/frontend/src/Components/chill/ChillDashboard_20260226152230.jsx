...
function ChillDashboard() {
  ...
  const fetchAllReels = async () => {
    try {
      setLoading(true);
      // Using your backend endpoint /api/chills/reels
      const { data } = await axios.get('/api/chills/reels?limit=100');
...
const handleUploadReel = async (e) => {
  ...
  // IMPORTANT: Use 'file' to match backend's req.file
  formData.append('file', selectedVideo);  // Changed from 'video' to 'file'
  formData.append('caption', caption);

  try {
    setUploading(true);
    const { data } = await axios.post('/api/chills/reels', formData, {
      headers: { 
  ...
  const handleLike = async (reelId, isLiked) => {
    try {
      const { data } = await axios.put(`/api/chills/reels/${reelId}/${isLiked ? 'unlike' : 'like'}`);
  ...
  const handleSave = async (reelId, isSaved) => {
    try {
      const { data } = await axios.put(`/api/chills/reels/${reelId}/${isSaved ? 'unsave' : 'save'}`);
