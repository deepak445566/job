...
const ReelItem = React.memo(({ reel, index, isGloballyMuted, onLike, onComment, onSave, isVisible, shouldPlay }) => {
  ...
  // Handle play/pause based on visibility and shouldPlay
  useEffect(() => {
...
function ChillDashboard() {
  ...
  // =========================
  // DEBOUNCED SCROLL HANDLER (FIXED)
  // =========================
  ...
  // =========================
  // INTERSECTION OBSERVER FOR VIDEOS (FIXED)
  // =========================
  useEffect(() => {
      ...
      (entries) => {
        ...
        entries.forEach((entry) => {
          ...
          if (entry.isIntersecting && entry.intersectionRatio >= 0.6) {
            // PAUSE ALL OTHER VIDEOS - THIS IS CRITICAL
  ...
  }, [reels, showComments, uploadModal, debouncedSetCurrentIndex]);

  // =========================
  // ENSURE ONLY CURRENT VIDEO PLAYS (ADDED)
  // =========================
  useEffect(() => {
    if (!reels.length || showComments || uploadModal) return;
  ...
  }, [currentVideoIndex, reels.length, showComments, uploadModal]);
      ...
      >
        {visibleReels.map((reel, index) => (
          <ReelItem
            ...
            onSave={handleSave}
            isVisible={index === currentVideoIndex || Math.abs(index - currentVideoIndex) <= 1}
