...
function ChillDashboard() {
  ...
  const handleScreenClick = useCallback((e) => {
    ...
    // Don't toggle if comments panel is open
    if (showComments) {
      return;
    }
    toggleMute();
  }, [toggleMute, showComments]);
  ...
  // LIKE HANDLER
  // =========================
  const handleLike = async (reelId, index, e) => {
    e.preventDefault();
    ...
    >
      ...
      >
        {reels.map((reel, index) => (
          ...
          >
            {/* Video */}
           // Update the video element to prevent default click behavior
<video
  ref={(el) => {
  ...
  onClick={(e) => e.stopPropagation()} // Add this to prevent video click behavior
/>

// Also update the like button handler to ensure propagation is stopped
const handleLike = async (reelId, index, e) => {
  e.preventDefault();
  e.stopPropagation(); // Already have this, keep it
  e.nativeEvent.stopImmediatePropagation(); // Add this for extra safety
...
>
  {/* button content */}
            ...
            <div className="absolute right-0 bottom-5 flex flex-col items-center gap-6 z-10">
              ...
              <button
                className="flex flex-col items-center group"
                onClick={(e) => handleLike(reel._id, index, e)}
              >
      ...
      {/* Comments Side Panel - WHITE BACKGROUND */}
      {showComments && selectedReel && (
        <>
          {/* Overlay */}
          <div 
            className="fixed inset-0 bg-black bg-opacity-50 z-[100]"
            onClick={(e) => handleCloseComments(e)}
          />
          
          {/* Side Panel - White Background */}
          <div className="fixed top-0 right-0 h-full w-full max-w-md bg-white z-[101] shadow-xl transform transition-transform duration-300 ease-in-out">
            {/* Header */}
              ...
              <form onSubmit={handleAddComment}>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-gray-100 text-gray-900 px-4 py-2 rounded-full outline-none focus:ring-2 focus:ring-purple-500"
                  />
                  <button
                    type="submit"
