...
const ReelItem = React.memo(({ reel, index, isGloballyMuted, onLike, onComment, onSave, isActive }) => {
  ...
  // CRITICAL: Sirf active reel chale, baaki sab pause
  useEffect(() => {
    if (!videoRef.current) return;

    if (isActive) {
      // Active reel - play karo
...
function ChillDashboard() {
  ...
  const [activeVideoIndex, setActiveVideoIndex] = useState(0); // Sirf active reel ka index
  ...
  // =========================
  // INTERSECTION OBSERVER - SIRF ACTIVE INDEX UPDATE KAREGA
  // =========================
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        // Sabse zyada visible reel dhundo
        let maxVisibility = 0;
        ...
        // Agar visibility 50% se zyada hai to active index update karo
        if (maxVisibility >= 0.5 && mostVisibleIndex !== activeVideoIndex) {
  ...
  }, [reels, showComments, uploadModal, activeVideoIndex]);
  ...
  }, [uploadModal, showComments, pauseAllVideos]);
      ...
      >
        {reels.map((reel, index) => (
          <ReelItem
            ...
            onSave={handleSave}
            isActive={index === activeVideoIndex} // SIRF YEH TRUE HOGA ACTIVE REEL KE LIYE
          />
        ))}
