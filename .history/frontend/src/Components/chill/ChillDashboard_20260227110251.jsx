...
function ChillDashboard() {
  ...
  const fetchReels = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        videoRefs.current = videoRefs.current.slice(0, res.data.reels.length);
      }
    } catch (error) {
      console.log(error);
    } finally {
  ...
  useEffect(() => {
    ...
    const options = {
      ...
      rootMargin: "0px",
      threshold: 0.6, // 60% video visible hoga tab play karega
    };
  ...
  }, [reels]); // Re-run when reels change
  ...
  return (
    <div className="bg-black text-white min-h-screen relative">
      ...
      <div 
        ...
        onScroll={() => {
          // Force check visibility on scroll
          videoRefs.current.forEach((video, index) => {
            if (video) {
              const rect = video.getBoundingClientRect();
              const isVisible = rect.top < window.innerHeight && rect.bottom > 0;
              if (isVisible && index === Math.floor(window.scrollY / window.innerHeight)) {
                video.play().catch(() => {});
              }
            }
          });
      ...
      >
        {reels.map((reel, index) => (
          ...
          >
            ...
            {/* Video Index Indicator (Current playing) */}
            {currentVideoIndex === index && (
              <div className="absolute top-4 left-4 bg-black bg-opacity-50 px-2 py-1 rounded text-xs">
                Playing
              </div>
            )}
          </div>
